...
switch upper(modem.fec_type)
    case 'LDPC'
        ...
        dec = comm.LDPCDecoder(cfg, 'DecisionMethod', 'Soft decision', ...
            'OutputFormat', 'Information bits', 'IterationTerminationCondition', 'Parity check satisfied');
        blk_out = dec.NumOutputBits;
    ...
    case 'TURBO'
        dec = comm.TurboDecoder( ...
            ...
            'NumIterations', 6);
        llr  = zero_pad(llr, 3*modem.block_len + 4*6);
        decoded = dec(llr(:));

    case 'CONV'
        trel    = poly2trellis(7, [171 133]);
        decoded = vitdec(double(llr < 0), trel, 34, 'cont', 'hard');
        decoded = decoded(35:end);  % Remove Viterbi delay
